...
import 'package:offline_note/data/repository/note_repo.dart';
...
class ViewNoteBloc implements BlocBase {
  ...
  void _handleSaveNote(Note note) async {
    await NoteRepository.getProvider().updateNote(note);
  }

  void _handleDeleteNote(int id) async {
    await NoteRepository.getProvider().deleteNote(id);
